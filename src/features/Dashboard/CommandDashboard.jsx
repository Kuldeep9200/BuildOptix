...
export default function CommandDashboard() {
  ...
  // मॉक फंक्शन्स
  const kpiNav = (target, index, e) => console.log(`Navigating to ${target}, tab: ${index}`);
  const switchToEquipment = (equip) => console.log(`Switching to equipment: ${equip}`);
  const toast = (msg, type) => alert(`${type.toUpperCase()}: ${msg}`);
  const navTo = (target, e) => console.log(`Navigating to ${target}`);
      ...
      {activeTab === 0 && (
        <div className={`tab-panel ${activeTab === 0 ? 'active' : ''}`} data-page="command" data-tab="0">
          ...
          <div className="kpi-strip mb-14" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
            <div className="kpi glow-warn clickable" title="Go to System Health" onClick={(e) => kpiNav('syshealth', 0, e.currentTarget)}>
              <div className="kpi-l">Overall Score</div>
              <div className="kpi-v warn">92.4</div>
              <div className="kpi-s">↑ 2.1% vs last month</div>
            </div>
            <div className="kpi glow-info clickable" title="Go to Complaints & FM" onClick={(e) => kpiNav('complaints', 0, e.currentTarget)}>
              <div className="kpi-l">Open Tickets</div>
              <div className="kpi-v">47</div>
              <div className="kpi-s">↓ 12 since yesterday</div>
            </div>
            <div className="kpi glow-bad clickable" title="Go to Alerts — Critical" onClick={(e) => kpiNav('alerts', 1, e.currentTarget)}>
              <div className="kpi-l">Critical Alerts</div>
              <div className="kpi-v bad">3</div>
              <div className="kpi-s">— unchanged</div>
            </div>
            <div className="kpi glow-ok clickable" title="Go to SLA Tracker" onClick={(e) => kpiNav('sla', 0, e.currentTarget)}>
