...
export default function Predictive_Risk() {
  ...
  return (
    <div className="page" id="pg-ai">
      ...
      <div className="tab-navigation" style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
        {["Summary", "Risk Matrix", "Root Cause", "Optimization", "Timeline", "Maintenance"].map((tabName, index) => (
          <button 
            key={index} 
            className={`aip-btn ${activeTab === index ? 'go' : ''}`}
